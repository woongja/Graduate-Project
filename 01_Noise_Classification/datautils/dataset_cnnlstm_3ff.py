"""
CNNLSTM_3FF Dataset — Spectrogram + MFCC + F0 for 3-Feature Fusion
  - 출력: (spec, mfcc, f0, label)
    - spec: (1, n_mels, T) mel spectrogram
    - mfcc: (1, n_mfcc, T) MFCC coefficients
    - f0: (1, T) fundamental frequency contour
  - sample_rate: 22050 Hz
  - n_mels: 128, n_mfcc: 40, n_fft: 1024, hop_length: 512
"""
import random
import torch
import torchaudio
...
class CNNLSTM_3FF_Dataset(Dataset):
    """Dataset for 3-Feature Fusion: Spectrogram + MFCC + F0"""
    ...
    def __init__(
        ...
        print(f"[CNNLSTM_3FF_Dataset] Loaded {len(self.samples)} samples (split={split})")
    ...
    def extract_f0_yin(self, waveform: torch.Tensor) -> torch.Tensor:
        """
        Extract F0 using YIN algorithm (simplified autocorrelation-based)
    ...
    def __getitem__(self, idx):
        ...
        # Use spec's time dimension as reference
        T_target = T_spec

        # Interpolate MFCC
        if T_mfcc != T_target:
            mfcc = torch.nn.functional.interpolate(
                mfcc.unsqueeze(0),  # (1, 1, n_mfcc, T_mfcc)
                size=(N_MFCC, T_target),
                mode='bilinear',
                align_corners=False
            ).squeeze(0)  # (1, n_mfcc, T_target)
        ...
        if T_f0 != T_target:
            f0 = torch.nn.functional.interpolate(
                ...
                mode='linear',
                align_corners=False
            ).squeeze(0)  # (1, T_target)

        return spec, mfcc, f0, label


# Collate function for DataLoader
def collate_3ff(batch):
    """
    Custom collate function for variable-length sequences

    Args:
        batch: List of (spec, mfcc, f0, label) tuples
    Returns:
        specs: (B, 1, n_mels, T_max)
        mfccs: (B, 1, n_mfcc, T_max)
        f0s: (B, 1, T_max)
        labels: (B,)
    """
    specs, mfccs, f0s, labels = zip(*batch)
    ...
    mfccs_batch = torch.stack(mfccs_padded)  # (B, 1, n_mfcc, T_max)
    f0s_batch = torch.stack(f0s_padded)      # (B, 1, T_max)
    labels_batch = torch.tensor(labels, dtype=torch.long)

    return specs_batch, mfccs_batch, f0s_batch, labels_batch
...
if __name__ == "__main__":
    ...
    if len(dataset) > 0:
        ...
        loader = DataLoader(dataset, batch_size=4, collate_fn=collate_3ff)
        batch = next(iter(loader))
        specs, mfccs, f0s, labels = batch
        print(f"\nBatch shapes:")
