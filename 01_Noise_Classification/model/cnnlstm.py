...
# Standalone CNN+LSTM Classifier (spectrogram only)
class CNNLSTMClassifier(nn.Module):
    """SpectrogramBranchLSTM + classification head.

    Input: mel spectrogram (B, 1, n_mels, T)
    """
    def __init__(self, num_classes: int, lstm_hidden: int = 128, lstm_layers: int = 2, dropout: float = 0.5):
        super().__init__()
        self.backbone = SpectrogramBranchLSTM(
            in_channels=1,
            output_dim=256,
